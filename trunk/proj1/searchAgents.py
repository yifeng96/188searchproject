...
def cornersHeuristic(state, problem):
  ...
  for bool in state[1]:
      if bool == True and other_dots != 0:
          other_dots -= min_side
    
  if state[1][0] == False:
      manhattan = abs(state[0][0] - corners[0][0]) + abs(state[0][1] - corners[0][1])
      if (closest == 0) or closest > manhattan:
          closest = manhattan
          
  if state[1][1] == False:
      manhattan = abs(state[0][0] - corners[1][0]) + abs(state[0][1] - corners[1][1]) 
      if (closest == 0) or closest > manhattan:
          closest = manhattan
          
  if state[1][2] == False:
      manhattan = abs(state[0][0] - corners[2][0]) + abs(state[0][1] - corners[2][1]) 
      if (closest == 0) or closest > manhattan:
          closest = manhattan
        
  if state[1][3] == False:
      manhattan = abs(state[0][0] - corners[3][0]) + abs(state[0][1] - corners[3][1]) 
      if (closest == 0) or closest > manhattan:
          closest = manhattan
  return closest + other_dots
